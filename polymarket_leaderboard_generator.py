...
DOMEAPI_KEY = os.getenv('DOMEAPI_KEY', '4d8e5410-e3bf-4abf-838b-0d3b0312bdd9')
...
def get_headers():
    ...
    if DOMEAPI_KEY:
        headers['Authorization'] = f'Bearer 4d8e5410-e3bf-4abf-838b-0d3b0312bdd9'
    return headers
...
def generate_leaderboard():
    ...
    for idx, market in enumerate(markets, 1):
        ...
        for order in orders:
            ...
            # Store trade data with all fields from order and market
            traders_data[wallet]['wallet_address'] = wallet
            
            # Start with all order fields (this includes order_hash, id, user, etc.)
            trade_data = dict(order)
            
            # Add calculated fields
            trade_data['pnl'] = pnl
            trade_data['volume'] = volume
            trade_data['result'] = result
            trade_data['winning_token_id'] = winning_token_id
            trade_data['winning_label'] = winning_label
            
            # Add market information (prefix with market_ to avoid conflicts)
            trade_data['market_slug'] = market_slug
            trade_data['market_title'] = market.get('title', 'Unknown')
            for key, value in market.items():
                if key not in ['slug', 'title']:  # Already added above
                    trade_data[f'market_{key}'] = value
            
            traders_data[wallet]['trades'].append(trade_data)
