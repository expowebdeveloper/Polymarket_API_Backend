...
async def process_trader_record(session: AsyncSession, trader_data: Dict, fetched_at: datetime) -> bool:
    ...
    try:
        ...
        result = await session.execute(
            text("SELECT id FROM daily_volume_leaderboard WHERE wallet_address = :wallet"),
            {"wallet": wallet_address}
        )
        ...
        if existing:
            ...
            return False # Updated, not inserted (technically) - keeping return semantic as "inserted new" = True? The caller counts "inserted" and "updated" for stats.
            # But wait, original code returned True for insert.
            # I should return True if inserted, False if updated?
            # The caller uses `if await process_trader_record(...)` -> increments `batch_inserted` and `total_inserted`.
            # I should probably change the signature to return (inserted, updated) like fetch_leaderboard_data.py does, or simpler:
            # caller: if await process...: batch_inserted += 1
            # If I return False for update, then stat "inserted" will filter out updates.
